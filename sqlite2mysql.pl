...
if (!$file) {
  print "usage: ./sqlite2mysql /path/to/fiile.sql > output.sql";
  exit;
}
...
while (my $line = <$info>) {
  if (($line !~  /BEGIN TRANSACTION/) && ($line !~ /COMMIT/) && ($line !~ /sqlite_sequence/) && ($line !~ /CREATE UNIQUE INDEX/) && ($line !~ /PRAGMA foreign_keys=OFF/)) {
    ...
    $line =~ s/notify not null default false/notify boolean not null default \'0\'/g;
    print $line;
    print "\n";
  }
}
